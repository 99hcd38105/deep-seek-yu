(() => {
  ...
  const cleanText = (value, maximum = 800) => String(value || '')
    .replace(/\u200b/g, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
    .slice(0, maximum);
  ...
  const extractMessages = () => {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      if (kind === 'user') {
        ...
        clone.querySelectorAll('[data-variant="think"],button,svg,[aria-label*="复制"],[aria-label*="copy" i]').forEach((node) => node.remove());
      ...
      messages.push({ key: `${kind}:${index}:${text.slice(0, 64)}`, role, text });
    }
  ...
  const installPetEntry = () => {
    ...
    button.textContent = '◉';
    button.title = '显示桌宠';
    button.setAttribute('aria-label', '显示桌宠');
