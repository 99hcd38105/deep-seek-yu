(() => {
  ...
  const installWindowControls = () => {
    if (mobileClient || document.getElementById('deep-seek-yu-titlebar-space')) return;
    if (!document.getElementById('deep-seek-yu-window-controls-style')) {
      const style = document.createElement('style');
      style.id = 'deep-seek-yu-window-controls-style';
      style.textContent = `html{height:100%;padding-top:36px;box-sizing:border-box;background:var(--dsw-alias-bg-base,#fff)}body{height:100%;margin:0}#deep-seek-yu-titlebar-space{position:fixed;z-index:2147483645;left:0;right:0;top:0;height:36px;background:var(--dsw-alias-bg-base,#fff);border-bottom:1px solid var(--dsw-alias-border-l2,#e2e8f0);-webkit-app-region:drag;pointer-events:none}`;
      document.head.append(style);
    }
    document.body.append(Object.assign(document.createElement('div'), { id: 'deep-seek-yu-titlebar-space' }));
  };
  ...
  const installSidebarBalance = () => {
    ...
    if (!document.getElementById('deep-seek-yu-sidebar-balance-style')) {
      ...
      style.textContent = `.hHd-Xa_settingsArea{display:grid!important;grid-template-columns:minmax(88px,1fr) minmax(88px,1fr);align-items:center;gap:6px}.hHd-Xa_settingsArea>*{min-width:0!important;width:auto!important}#deep-seek-yu-sidebar-balance{min-width:88px!important;height:34px;display:flex;align-items:center;justify-content:flex-end;gap:5px;padding:0 8px;border:0;border-radius:9px;background:transparent;color:var(--dsw-alias-label-secondary,#475569);cursor:pointer;font:600 12px/1.2 system-ui;white-space:nowrap;overflow:hidden}#deep-seek-yu-sidebar-balance:hover{background:var(--dsw-alias-interactive-bg-hover,#eef2f7);color:var(--dsw-alias-label-primary,#111827)}#deep-seek-yu-sidebar-balance span{overflow:hidden;text-overflow:ellipsis}.hHd-Xa_collapsed .hHd-Xa_settingsArea{display:flex!important}.hHd-Xa_collapsed .hHd-Xa_settingsArea>*{width:36px!important}.hHd-Xa_collapsed #deep-seek-yu-sidebar-balance{display:none!important}`;
      document.head.append(style);
    }
    const button = document.createElement('button');
  ...
  const renderPluginPanel = (panel) => {
    ...
    panel.innerHTML = `
      ...
      <div class="dsy-grid">
        ${mobileClient ? '' : '<section class="dsy-card wide" data-card="client"><h4>客户端与手机连接</h4><div class="dsy-muted">原窗口顶部菜单已经移除，常用功能集中在这里。DeepSeek 官方模型每 15 分钟自动同步。</div><div class="dsy-actions" style="margin-top:12px"><button class="dsy-button" data-client-back>返回</button><button class="dsy-button" data-client-refresh>刷新</button><button class="dsy-button" data-client-api-key>更换 API Key</button><button class="dsy-button" data-client-models>立即同步官方模型</button><button class="dsy-button" data-mobile-toggle>开启手机连接</button><button class="dsy-button" data-mobile-copy>复制连接地址</button><button class="dsy-button" data-mobile-reset>重置连接地址</button><button class="dsy-button" data-client-fullscreen>全屏</button><button class="dsy-button" data-client-zoom-out>缩小</button><button class="dsy-button" data-client-zoom-reset>100%</button><button class="dsy-button" data-client-zoom-in>放大</button></div><div class="dsy-state" data-client-state>正在读取客户端状态…</div></section>'}
    ...
    if (!mobileClient) {
      ...
      clientAction('[data-client-models]', 'client:models-refresh', '已从 DeepSeek 官方同步最新模型，新建会话即可使用。');
